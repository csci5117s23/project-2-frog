    ...
    return <div className='card' css={css`margin: 0.25rem;`}>
        <Link href={`/plants/${props.plant._id}`}>
            ...
            <div className={emergency ?  'card-content has-background-danger' : 'card-content'}>
                <h1 className= {emergency ?  'title has-text-centered has-text-white' : 'title has-text-centered'} 
                    css={css`height: 3rem;`}>{props.plant.name || 'Unnamed Plant'}</h1>
                <p className= {emergency ?  'subtitle has-text-centered has-text-white' : 'subtitle has-text-centered'} >Needs water in {water} days</p>
            </div>
        </Link>
    </div>
}